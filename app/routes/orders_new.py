...
ORDER STATUS FLOW:
    pending_payment → confirmed → preparing → ready → picked_up → delivered
    ...
    ready:          Food ready for pickup by rider
    picked_up:      Rider has picked up the food
    delivered:      Order delivered to customer

ROLE-BASED STATUS TRANSITIONS:
    - CUSTOMER:     Can only view their orders
    - RESTAURANT:   confirmed→preparing, preparing→ready
    - RIDER:        ready→picked_up, picked_up→delivered
    - SYSTEM:       pending_payment→confirmed (via payment webhook)
"""
...
def get_available_orders_for_riders():
    """
    Get all orders ready for pickup (status=ready, no rider assigned).
...
def assign_rider_to_order(order_id):
    ...
    try:
        ...
        if order["status"] != "ready":
            return jsonify(
                {
                    "error": f"Order must have status 'ready' to accept. Current status: '{order['status']}'"
                }
            ), 400
